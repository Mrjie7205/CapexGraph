...
- 计划状态：技术交付完成并并入`v0.5.1`；默认主线阈值保持`experimental`
- 基线版本：`v0.3.0`
- 完成分支：`codex/pre-v0-6-completion`
- 最后更新：2026-08-03

> 完成说明：M0-M5的契约、迁移、Provider、点时主题注册表、确定性指标、幂等任务、
> 人工门控重评和Cockpit路径均已实现。用户尚未单独批准一套生产投资阈值，因此内置
> 保守政策以版本化`experimental`状态交付；它可用于演示、回归和人工研究，不会被
> 描述为自动买卖规则。
...
| Yahoo | 显式无Key网络路径 | 手工选择的演示和有限研究路径 | 非正式、无SLA，不作为生产默认质量基准，也不被静默回退 |
...
- [x] 市场和主题契约、迁移及Provider能力声明完成；
- [x] EODHD中美韩日线接入和质量门禁完成；
- [x] A股增强/校验源Tushare适配器完成；
- [x] 主题定义、双时间成员和历史快照可查询；
- [x] CN、US、KR冻结历史成分路径完成；
- [x] ETF/指数认知与证据化产业暴露在Schema和UI中分离；
- [x] 主题指标对点时股票池确定性计算；
- [x] 主线分类、阈值和生效日版本化，且未获产品批准前显式标记`experimental`；
- [x] 盘后任务幂等、失败隔离且默认不触发模型费用；
- [x] 状态变化和链接重评不修改原始运行；
- [x] Cockpit可以解释来源、覆盖、质量、指标和状态变化；
- [x] 无Key黄金路径、完整测试、Web build和wheel纳入发布门禁；
- [x] 商业Key与授权原始数据没有进入Git或公开fixture；
- [x] 文档明确v0.5已交付、v0.6共识预期能力尚未实现。
